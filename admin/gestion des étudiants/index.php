...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && isset($_POST["id"])) {
    $id = $_POST["id"];
    $sql = "SELECT semester FROM etudiant WHERE id='$id'";
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        ...
        // Mettre à jour le statut de leader dans la table "etudiant"
        $sql_update = "UPDATE `etudiant` SET `lead` = $new_lead_status WHERE `id` = $id";
        mysqli_query($conn, $sql_update);

        if ($new_lead_status == 1) {
            // Si l'utilisateur est désormais un leader, insérer une entrée dans la table "group"
            $sql_insert_group = "INSERT INTO `group` (id_etudiant, semester) VALUES ($id, '$semester')";
            mysqli_query($conn, $sql_insert_group);

            // Insérer une entrée dans la table "membres_group"
            $group_id = mysqli_insert_id($conn);
            $sql_insert_member = "INSERT INTO `membres_group` (id_etudiant, id_group, semester) VALUES ($id, $group_id, '$semester')";
            mysqli_query($conn, $sql_insert_member);
        } else {
            // Si l'utilisateur n'est plus un leader, supprimer l'entrée correspondante de la table "membres_group" d'abord
            $sql_select_member = "SELECT id_group FROM `membres_group` WHERE id_etudiant = $id";
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                ...
                // Supprimer l'entrée correspondante de la table "membres_group"
                $sql_delete_member = "DELETE FROM `membres_group` WHERE id_group = $group_id";
                mysqli_query($conn, $sql_delete_member);

                // Ensuite, supprimer l'entrée correspondante de la table "group"
                $sql_delete_group = "DELETE FROM `group` WHERE id = $group_id";
                mysqli_query($conn, $sql_delete_group);
            }
        }
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-danger"
                            ...
                            data-bs-target="#deleteModal_<?php echo $row["id"] ?>">
